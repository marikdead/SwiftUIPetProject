# FirstPetProject — учебный проект по Swift с нуля

Этот README написан как мини-курс по твоему текущему приложению.  
Цель: чтобы ты не просто “запускал код”, а понимал **почему он устроен именно так** и мог повторить проект самостоятельно.
...
## Главная идея архитектуры (самое важное)
...
## 13) Где смотреть в первую очередь (маршрут изучения)

Если ты только начал Swift:

1. `FirstPetProjectApp.swift` — пойми вход и навигацию.
2. `Domain/Entities/TipCalculation.swift` — простая чистая математика.
3. `Domain/UseCases/CalculateTipUseCase.swift` — зачем нужен UseCase.
4. `Presentation/ViewModel/TipCalculatorViewModel.swift` — как связываются UI и бизнес-логика.
5. `Presentation/Views/TipCalculatorView.swift` — как строится SwiftUI экран.
6. `Presentation/ViewModel/MenuViewModel.swift` + `MenuView.swift` — более крупный сценарий со списком и состоянием заказа.

---

## 14) Что можно улучшить в следующем шаге (для роста)

- добавить unit-тесты для `CalculateTipUseCase` и `TipCalculatorViewModel`;
- вынести `Color(hex:)` в отдельный файл `Presentation/Theme/Color+Hex.swift`;
- вынести форматирование денег в отдельный сервис;
- подключить реальные данные меню (JSON/API) вместо `MenuRepository.items`;
- добавить локализацию (`Localizable.strings`) и переключение языка.

---

## 15) Быстрый FAQ

**Почему ViewModel не импортирует SwiftUI?**  
Чтобы не зависеть от UI и легче тестироваться.

**Почему есть протокол UseCase, если можно сразу класс?**  
Чтобы легко подменять реализацию в тестах и держать зависимости абстрактными.

**Где менять внешний вид?**  
В `Presentation/Views/*` файлах.

**Где менять логику расчета?**  
В `Domain/UseCases` и `Domain/Entities`.

---

Если хочешь, следующим шагом могу добавить в этот README:
- диаграмму “слои и зависимости” (в ASCII);
- раздел “как писать первые unit tests в Xcode” с готовыми тест-кейсами под твой код;
- чеклист “как добавить новую фичу без поломок архитектуры”.
